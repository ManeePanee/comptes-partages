...
interface Props {
  expenses: Expense[]
  defaultFilter?: Person | 'all'
  showFilter?: boolean
}

export function CategoryPieChart({ expenses, defaultFilter = 'all', showFilter = true }: Props) {
  const [filter, setFilter] = useState<Person | 'all'>(defaultFilter)
  ...
  return (
    <div>
      ...
      {data.length === 0 ? (
        <div className="flex items-center justify-center h-40 text-sm text-[var(--brown-300)]">
          Aucune dépense {filter !== 'all' ? `pour ${filter === 'mane' ? 'Mane' : 'Myriem'}` : ''}
        </div>
      ) : (
        <>
